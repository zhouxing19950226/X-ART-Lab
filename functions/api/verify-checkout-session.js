...
export async function onRequestGet({ request, env }) {
  if (!env.STRIPE_SECRET_KEY) return json({ active: false, error: "Stripe 未配置" }, 503);
  ...
  if (!stripeResponse.ok) return json({ active: false }, 404);
  ...
  return json({
    ...
    plan: subscription?.metadata?.xart_plan || null,
