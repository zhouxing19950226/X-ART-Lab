...
function blackLogoCanvas(image){const canvas=document.createElement("canvas");canvas.width=image.naturalWidth||image.width;canvas.height=image.naturalHeight||image.height;const context=canvas.getContext("2d",{willReadFrequently:true});context.drawImage(image,0,0);const pixels=context.getImageData(0,0,canvas.width,canvas.height);for(let index=0;index<pixels.data.length;index+=4){const alpha=pixels.data[index+3],lightness=(pixels.data[index]+pixels.data[index+1]+pixels.data[index+2])/3;if(!alpha)continue;if(lightness>245){pixels.data[index+3]=0;continue}pixels.data[index]=0;pixels.data[index+1]=0;pixels.data[index+2]=0}context.putImageData(pixels,0,0);return canvas}
